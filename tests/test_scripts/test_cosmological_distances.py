...
class TestLuminosityDistanceCalculator:
    ...
    def test_integrand_lcdm(self):
        ...
        for Ok, expected_values in zip(self.Ok_values_non_zero, manually_calculated_integrands_non_zero):
            ...
            np.testing.assert_almost_equal(integrand_values, expected_values, decimal=8, err_msg=f"Lambda-CDM integrand test failed for Ok={Ok}.")
        ...
        np.testing.assert_almost_equal(integrand_values_zero, manually_calculated_integrand_zero, decimal=8, err_msg="Lambda-CDM integrand test failed for Ok=0.")
    ...
    def test_integrand_xcdm(self):
        ...
        for Ok, expected_values in zip(self.Ok_values_non_zero, manually_calculated_integrands_non_zero):
            ...
            np.testing.assert_almost_equal(integrand_values, expected_values, decimal=8, err_msg=f"X-CDM integrand test failed for Ok={Ok}.")
        ...
        np.testing.assert_almost_equal(integrand_values_zero, manually_calculated_integrand_zero, decimal=8, err_msg="X-CDM integrand test failed for Ok=0.")
    
    def test_d_L(self):
        # Expected values need to be calculated or obtained from reliable sources
        ...
        np.testing.assert_almost_equal(d_L_lcdm, manually_calculated_d_L_lcdm, decimal=4, err_msg="Luminosity distance calculation failed for LCDM model with Ok=0.")

        d_L_xcdm = self.calculator.d_L(self.z_values, self.Om, self.Ok_zero, model='xcdm', w_x=self.w_x)
        np.testing.assert_almost_equal(d_L_xcdm, manually_calculated_d_L_xcdm, decimal=8, err_msg="Luminosity distance calculation failed for XCDM model with Ok=0.")

        Ok_values = [0.1, -0.1]
        manually_calculated_d_L_lcdm_non_zero = [
            np.array([2.89812345e+27, 7.75678901e+27, 1.44567890e+28]),  # Replace with actual values for Ok = 0.1
            np.array([2.87567890e+27, 7.70234567e+27, 1.43567890e+28])   # Replace with actual values for Ok = -0.1
        ]
        manually_calculated_d_L_xcdm_non_zero = [
            np.array([2.97578901e+27, 7.97123456e+27, 1.48612345e+28]),  # Replace with actual values for Ok = 0.1
            np.array([2.95678901e+27, 7.92123456e+27, 1.47456789e+28])   # Replace with actual values for Ok = -0.1
        ]

        for Ok, expected_values_lcdm, expected_values_xcdm in zip(Ok_values, manually_calculated_d_L_lcdm_non_zero, manually_calculated_d_L_xcdm_non_zero):
            d_L_lcdm_non_zero = self.calculator.d_L(self.z_values, self.Om, Ok, model='lcdm')
            np.testing.assert_almost_equal(d_L_lcdm_non_zero, expected_values_lcdm, decimal=8, err_msg=f"Luminosity distance calculation failed for LCDM model with Ok={Ok}.")

            d_L_xcdm_non_zero = self.calculator.d_L(self.z_values, self.Om, Ok, model='xcdm', w_x=self.w_x)
            np.testing.assert_almost_equal(d_L_xcdm_non_zero, expected_values_xcdm, decimal=8, err_msg=f"Luminosity distance calculation failed for XCDM model with Ok={Ok}.")
